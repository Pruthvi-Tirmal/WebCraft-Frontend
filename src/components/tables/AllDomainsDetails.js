...
import { getAllDomains } from '../../api/adminAPI';
...
const AllDomainsDetails = () => {
    ...
    const displayDomainSections = paginationAndSearch(pageNumber, domain, search, usersPerPage).map((item) => (
        <tr key={item._id}>
            <td className='text-gray-800 border py-4 px-6'>{item.loggedUser}</td>
            <td className='text-gray-800 border py-4 px-6'>{item.domain}</td>
    ...
    // page count 
    const pageCount = Math.ceil(domain.length / usersPerPage);
    ...
    return (
        <div>
            {loading ? (<Loader />) : (<div>
                <div className="overflow-auto relative h-[100vh]">
...
export default AllDomainsDetails